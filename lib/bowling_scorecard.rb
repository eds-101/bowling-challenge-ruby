...
# Runs game of bowling through user inputs into record_roll method
class BowlingScorecard
  ...
  def record_roll(input)
    ...
    if frame_ten_no_bonus?
      @scorecard[10] << 0
      return 'Gutter Game!' if total_score.zero?
    end
  end
  ...
  def process_strike_frame_ten
    add_to_bonus_queue(@frame, :strike, 0)
    @rolls_in_frame = 0 # to allow for another strike
  end

  def process_bonuses(bonus_score)
    queue = @bonus_queue.select { |_k, v| (v[:rolls_left]).positive? } # filter queue where rolls left > 0
    # add bonus, reduce rolls left to process
    queue.each do |frame, _values|
  ...
  def total_score
    sum = 0
    @scorecard.each do |_frame, score|
      score.each do |value|
